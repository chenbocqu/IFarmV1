...
public class StateLine extends View {
    ...
    private String[] data = {"任务下发", "开始", "完成"};
    ...
    private int unSelectedLineColor = Color.WHITE;
    private int unSelectedCircleColor = Color.WHITE;

    //字体大小
    private int mTextSize = (int) TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_SP, 15, getResources().getDisplayMetrics());

    private Drawable mTextBackground = getContext().getResources().getDrawable(R.drawable.label_bg);//选中的文字背景
        ...
        for (int i = 0; i < a.getIndexCount(); i++) {
            ...
            switch (attr) {
                case R.styleable.StateLine_selectedCircleColor:
                    ...
                    break;
                case R.styleable.StateLine_unSelectedCircleColor:
                    unSelectedCircleColor = a.getColor(attr, Color.WHITE);
                    break;
                case R.styleable.StateLine_selectedLineColor:
                    selectedLineColor = a.getColor(attr, Color.RED);
                    break;
                case R.styleable.StateLine_unSelectedLineColor:
                    unSelectedLineColor = a.getColor(attr, Color.WHITE);
                    break;
                case R.styleable.StateLine_text_size:
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < data.length; i++) {
            if (i < pointPosition) {
                ...
                mCirclePaint.setColor(selectedCircleColor);
                mLinePaint.setColor(unSelectedLineColor);
            } else {
                mCirclePaint.setColor(unSelectedCircleColor);
                mLinePaint.setColor(unSelectedLineColor);
            }
            canvas.drawCircle(5 + getPaddingLeft() + i * width, 5 + getPaddingTop(), 10, mCirclePaint);
            //最后一个点之后不再画线
            if (i != data.length - 1) {
...
//                mTextBackground.draw(canvas);
                ...
                mTextPaint.setColor(unSelectedCircleColor);
                mTextPaint.setTextSize(mTextSize);
            }
